...
# see: https://github.com/boto/boto3/issues/665    
    ...
    def get_db_records_by_secondary_key(self, sk, pvalue_condition=None):
        self.logger.debug("get by secondary key. sk: {}, condition: {}".format(sk, pvalue_condition))
        params = {
            "IndexName": "gsi_1",
    ...
    def get_db_record(self, pk, sk):
        try:
            ...
            if "Item" in db_record:
                ...
                self.fix_float(db_record)
                # self.logger.debug("result after normalization: {}".format(db_record))
    ...
    def fix_float(self, data):
        self.logger.debug("fix numbers in: {}".format(data))
        ...
        for i,value in iterator:
            if isinstance(value,(list,dict)):
                self.fix_float(value)
            elif isinstance(value,Decimal):
                try:
                    data[i] = float(value)
                    # self.logger.debug("fixed float: {} to {}".format(value, data[i]))
                except ValueError:
                    pass
